...
const Index = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if(res.status === 200){
const agancyUsers = res.data.users.filter((user)=>user.role === "GAA")
  ...
  const handleUserClick = (user) => {
    navigate(`/app/user/${user._id}`, { state: { user } });
  };
  ...
  return (
    <>
      <TitleCard title={'Agency Account'} topMargin="mt-2" TopSideButtons={<TopSideButtons applySearch={applySearch} />} >
                    ...
                    <tr key={user._id}>
                      <td>
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <Link to={`/app/user/${user._id.toString()}`}  className="font-bold cursor-pointer">{user.accountName}</Link>
                          </div>
                        </div>
                      </td>
                      <td>{user.email}</td>
