...
import EditModelJobIntern from "./EditModelIntern.js"
import AddModalJobIntern from './ADmodelintern.js';
...
const InternShipIndex = () => {
  ...
  const fetchData = async () => {
    ...
    const res = await axios.get(`https://arabic-application-01.vercel.app/api/jobs/personaluser/Allinternships`);
  ...
  const handleDelete = async (userId) => {
    try {
      await axios.delete(`https://arabic-application-01.vercel.app/api/jobs/del/iternee/${userId}`);
  ...
  return (
    <>
      <TitleCard title={'InternShip User'} topMargin="mt-2" TopSideButtons={<TopSideButtons applySearch={applySearch} HandleAddService={handleAddService} />} applySearch={applySearch} >
